...
import time 
import sys 
model = "../2pc_model_quantized_batches/build"
...
online_bs = 0
online_br = 0
if int(sys.argv[1]) > 400 or int(sys.argv[1]) < 1:
	sys.exit(-1)

subprocess.Popen(["./server", "-p", "9999", "-i", sys.argv[1]], stdout=subprocess.PIPE)

for i in range(repeat):
	print("{} / {}".format(i + 1,repeat))

	time.sleep(1)
	result = subprocess.check_output(["./client", "-p", "9999", "-i" ,sys.argv[1] ])
