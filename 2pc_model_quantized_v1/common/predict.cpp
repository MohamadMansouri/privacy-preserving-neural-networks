...
share* Shiffter(share* s_yh, const int shift)
{
	 ...
	 )
{
	...
	if (role == SERVER)
		{
			convert_vec_to_int(bias_arr_h,bias_all[0],nvals_h,48);
	...
	if (role == SERVER)
	{
		convert_vec_to_int(bias_arr_o,bias_all[1],nvals_o,48);
		convert_mat_to_int(weight_arr_o, weight_all[1],nvals_h,nvals_o,24);
	}
