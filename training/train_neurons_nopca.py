...
path = '../results/variable_neurons/nopca/'
...
def NN_model(act,neurons,vald,batch,epochs):

    model = Sequential()

    model.add(Dense(activation=act, input_dim=180,output_dim=neurons,
    ...
    history = model.fit(X_train_split, Y_train_split, epochs=epochs, shuffle=True,
...
def plot_confusion_matrix(cm, classes, name,
    ...
    fig_cm.savefig(path_to_cm + '{}.png'.format(name),dpi=300)
    plt.close(fig_cm)
...
def main():
    ...
    for act in activations:
        ...
        for neurons in range(2,102,2):
            ...
            plot_confusion_matrix(cnf_matrix,classes, neurons, True)
        ...
        fig.savefig(path + 'results_{}_{}_{}.png'.format(act,vald,
            batch),dpi=600)
        plt.close(fig)
        with open(path + 'results_{}_{}_{}.txt'.format(act,vald,batch), 'w')\
         as f:
            for i in evaluation:
