...
def main():
    ...
    for act in activations:
        ...
        for dim in range(4,45,2):
            ...
            fig_acc.savefig(path_to_accuracy + '{}.png'.format(dim))
            plt.close(fig_acc)
            ...
            plot_confusion_matrix(cnf_matrix,classes, dim, path_to_cm, True)

            # save the model parameters
            save_model(weight[0],bias[0], '{}_hidden'.format(dim))    
            save_model(weight[1],bias[1], '{}_out'.format(dim))    

        # draw the accuracy versus number of neurons
        fig = plt.figure(figsize=(14,10))
        ax = fig.add_subplot(1, 1, 1)
        x_ticks = np.arange(4, 45, 2)
        y_ticks = np.arange(0.75, 1.1, 0.02)
